...
public sealed class HybridSearchCoreTests
{
    ...
    [Fact]
    public async Task SupportsLexicalOnlySemanticOnlyAndProviderPolicies()
    {
        var id = Guid.NewGuid(); Assert.Equal(HybridSearchStatus.ProviderUnavailable, (await Engine([Hit(id, 10)], [], generators: []).SearchAsync(new("q"))).Status);
        Assert.Single((await Engine([Hit(id, 10)], []).SearchAsync(new("q"))).Results!);
        Assert.Single((await Engine([], [Semantic(id, 1)]).SearchAsync(new("q"))).Results!);
        Assert.Equal(HybridSearchStatus.Failed, (await Engine([], [], generators: [new FakeGenerator(), new FakeGenerator()]).SearchAsync(new("q"))).Status);
    }
    [Theory]
    ...
    { var a=Guid.NewGuid(); var b=Guid.NewGuid(); var partial=await Engine([Hit(a,10)],[],semanticStatus:SemanticSearchStatus.Failed).SearchAsync(new("q")); Assert.Equal(HybridSearchStatus.PartialSuccess,partial.Status); Assert.Equal("HYBRID_SEMANTIC_UNAVAILABLE",partial.Warnings![0].Code); var ranked=await Engine([Hit(b,10),Hit(a,10)],[]).SearchAsync(new("q")); Assert.Equal(new[]{a,b}.OrderBy(id=>id),ranked.Results!.Select(x=>x.MemoryId)); }
    ...
    private static HybridSearchEngine Engine(FakeSearch lexical, FakeSemantic semantic, IEnumerable<IEmbeddingGenerator>? generators=null) => new(lexical, semantic, generators ?? [new FakeGenerator()], Options.Create(new HybridSearchOptions()));
