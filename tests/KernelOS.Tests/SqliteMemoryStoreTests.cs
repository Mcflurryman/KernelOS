...
public sealed class SqliteMemoryStoreTests
{
    ...
    public async Task InitializerCreatesDatabaseSchemaAndIsIdempotent()
    {
        ...
        Assert.Equal(3L, Convert.ToInt64(await command.ExecuteScalarAsync(), CultureInfo.InvariantCulture));
    ...
    public async Task InitializerRejectsNewerSchemaVersions()
    {
        ...
        await using (var command = connection.CreateCommand())
        {
            command.CommandText = "UPDATE schema_version SET version = 4 WHERE singleton = 1;";
