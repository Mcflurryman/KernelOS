...
Document Readers Core recibe referencias autorizadas de Filesystem y produce `RawDocument` mediante Registry, Router, ReadService y Readers explícitos. Knowledge Core transforma ese resultado en `KnowledgeDocument` e items trazables. Memory Core In-Memory almacena snapshots de Knowledge mediante contratos, sin filesystem, LLM ni persistencia. TXT, Markdown, JSON y CSV están implementados; contenido documental no se interpreta como órdenes. Embeddings, RAG y búsqueda semántica siguen pendientes.
