...
El contenido sigue siendo no confiable: se conserva únicamente como `ContextItem.Content`, nunca como system prompt ni instrucción ejecutable. No hay llamadas a LLM, endpoints, Tools, Planner ni RAG. El builder y estimador son singletons sin estado por solicitud; la concurrencia hereda las garantías de `IMemoryStore`.
